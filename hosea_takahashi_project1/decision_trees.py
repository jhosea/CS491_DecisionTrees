...
def DT_train_binary(X,Y,max_depth):
	...
	if not max_depth == 0 and not X.size == 0:
		...
		if not entropy(left_labels) == 0:
			...
			#If there are no samples that fall to the left side return 0 for the prediction
		...
		if not entropy(right_labels) == 0:	
			...
			#If there are no samples that fall to the right side return 0 for the prediction
...
def DT_train_real(X,Y,max_depth):
	...
	if not max_depth == 0 and not X.size == 0:
		...
		if not entropy(left_labels) == 0:
			...
			#recursively calls DT_train_real on the left side of the split
			left_subtree = DT_train_real(left_training_data, left_labels, max_depth - 1)
		...
		#if the entropy is 0 then there is no need to continue splitting and we can make a decision
		if not entropy(right_labels) == 0:	
			#recursively calls DT_train_real on the right side of the split
			right_subtree = DT_train_real(right_training_data, right_labels, max_depth - 1)
